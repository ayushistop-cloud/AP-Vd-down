...
export const appErrors = {
    ...
    youtubeMetadataAccessFailed: (message = 'YouTube metadata extraction failed. Please try again shortly.') => new AppError('YOUTUBE_METADATA_ACCESS_FAILED', message, { retryable: true }),
    youtubeFormatAccessFailed: (message = 'YouTube format deciphering failed. Please try another quality option.') => new AppError('YOUTUBE_FORMAT_ACCESS_FAILED', message, { retryable: true }),
    youtubeStreamAccessFailed: (message = 'YouTube media stream could not be accessed. Please try again shortly.') => new AppError('YOUTUBE_STREAM_ACCESS_FAILED', message, { retryable: true }),
    youtubeAuthRequired: (message = 'YouTube requires authentication or member access to view this video.') => new AppError('YOUTUBE_AUTH_REQUIRED', message, { retryable: false }),
    extractorError: (message = 'Media could not be resolved right now. Please try again shortly.') => new AppError('EXTRACTOR_ERROR', message, { retryable: true }),
    authenticationRequired: (message = 'This content requires authentication or member access.') => new AppError('AUTHENTICATION_REQUIRED', message, { retryable: false }),
    runtimeFilesystemError: (message = 'Media processing engine encountered a runtime filesystem error. Please try again.') => new AppError('RUNTIME_FILESYSTEM_ERROR', message, { retryable: true }),
    processConfigurationError: (message = 'Media processing engine encountered a configuration error.') => new AppError('PROCESS_CONFIGURATION_ERROR', message, { retryable: true }),
